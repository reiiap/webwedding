const previewRoot = document.querySelector('[data-preview-root]') || document.body;
const params = new URLSearchParams(window.location.search);
const fallbackGuest = previewRoot.dataset.guest || 'Tamu Undangan';
const pathGuest = decodeURIComponent(window.location.pathname.split('/').filter(Boolean).pop() || '').replace(/[-_]+/g, ' ');
const guestName = params.get('to') || (pathGuest && pathGuest !== previewRoot.dataset.theme ? pathGuest : fallbackGuest);
...
const targetDate = new Date(previewRoot.dataset.eventDate || '2026-12-19T09:00:00+07:00');
